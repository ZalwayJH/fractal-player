...
export const MenuItem = ({ title, artist, album, duration, albumCover }) => {
  return (
    <Suspense fallback={<LoadingSongs />}>
      <div className="w-full rounded-md  px-2 p-1 items-center text-white inline-grid  gap-4">
        ...
        <span className="col-start-2 truncate w-[15em]">
          <h3>{title}</h3>
          <h4 className="text-gray-500">{artist}</h4>
        </span>
        <h4 className="col-start-3 text-wrap w-[8em] truncate text-gray-400">{album}</h4>
        <p className="col-start-4 pr-3 text-gray-400">{duration}</p>
      </div>
    </Suspense>
  );
};
